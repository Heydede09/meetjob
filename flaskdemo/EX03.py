...
def index():
    ...
    #抓取最新的產品 5筆
    sql = "select name,price,link,photo_url from ttproduct order by id desc limit 5"
    cursor = db.conn.cursor()
    cursor.execute(sql)
    db.conn.commit()
    hot= cursor.fetchall()
    ...
    #抓取最新的新聞4筆
    sql = "select title,link,photo from tnews order by id desc limit 4"
    cursor = db.conn.cursor()
    cursor.execute(sql)
    db.conn.commit()
...
def product():
    ...
    sql = "select count(*) as allcount from ttproduct" # 統計全部的筆數
    cursor = db.conn.cursor()
    cursor.execute(sql)
    db.conn.commit()
    ...
    if page == None:
        ...
        if items == None and keyword == None:
            sql= "select * from ttproduct order by id desc limit 10"
        elif len(items) > 0 and len(keyword) == 0:
            #使用者查類別但沒有查關鍵字
            sql = "select * from ttproduct where itemtype='{}' limit 10" .format(items)
            
        elif items =="0" and len(keyword) > 0:
            #關鍵字查詢
            sql = " select * from ttproduct where name like '%{}%' limit 10".format(keyword)
            
        else:
            #使用者查類別及關鍵字
            sql = "select *from ttproduct where itemtype='{}' and name like '%{}%' limit 10".format(items,keyword)
        ...
        #使用者都沒有依條件查詢，只單獨點選產品頁的連結
        if items == None and keyword == None:
            sql= "select * from ttproduct order by id desc limit {},{}".format(startPage*10,10)
        elif len(items) > 0 and len(keyword) == 0:
            #使用者查類別但沒有查關鍵字
            sql = "select * from ttproduct where itemtype='{}' limit {},{}" .format(items,startPage*10,10)
            
        elif items =="0" and len(keyword,startPage*10,page*10) > 0:
            #關鍵字查詢
            sql = " select * from ttproduct where name like '%{}%' limit {},{}".format(keyword,startPage*10,10)
            
        else:
            #使用者查類別及關鍵字
            sql = "select *from ttproduct where itemtype='{}' and name like '%{}%' limit {},{}".format(items,keyword,startPage*10,10)
