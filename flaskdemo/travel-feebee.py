...
for mySoup in soupS.find_all("li"):
    ...
    sql = "select * from ttproduct where name='{}' and itemtype=1 ".format(title)
    
    cursor.execute(sql)
    db.conn.commit()
    
    if cursor.rowcount ==0 : #表示沒有該產品
        sql = "insert into ttproduct(itemtype,name,price,link,photo_url) values('1','{}','{}','{}','{}')".format(title,price,link,photo)
        cursor.execute(sql)
        db.conn.commit()
